...
const WorkFlow = () => {
  return (
    <Box as='section' sx={styles.workflow}>
      <Container>
        <BlockTitle sx={styles.workflow.blockTitle} tagline='Whats the function' heading='Let’s see how it works' />
          ...
          {workflowData.map((item, index) => (
            <Box sx={styles.workflow.card} key={index}>
              ...
              <Box sx={styles.workflow.wrapper}>
                <Heading sx={styles.workflow.wrapper.title}>{item.title}</Heading>
  ...
  workflow: {
    ...
    card: {
      ...
      '&:nth-of-type(2n-1)::before': {
        backgroundImage: `url('arrow-odd')`
      },
      '&:nth-of-type(2n)::before': {
        backgroundImage: `url('arrow-even')`,
        top: 17
      },
      '&:last-child::before': {
        display: 'none'
